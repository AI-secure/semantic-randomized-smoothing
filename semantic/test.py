...
if __name__ == '__main__':
    ...
    for j in range(0, 100, 20):
        loader, set_name = test_dataset, 'test'
        orig = loader[j][0]
        transforms.visualize(orig, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{0}-orig.bmp')
        orig_mask = rotationT.masking(orig)
        rot = rotationT.raw_proc(orig, 15)
        transforms.visualize(rot, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{15}-orig.bmp')
        rot = rotationT.raw_proc(rot, 15)
        one_step = rotationT.raw_proc(orig, 30)
        transforms.visualize(rot, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{30}-orig.bmp')
        transforms.visualize(one_step, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{30}-recover.bmp')

        transforms.visualize(orig_mask, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{0}-masked-orig.bmp')
        rot = rotationT.raw_proc(orig_mask, 15)
        transforms.visualize(rot, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{15}-masked-orig.bmp')
        rot = rotationT.raw_proc(rot, 15)
        one_step = rotationT.raw_proc(orig_mask, 30)
        transforms.visualize(rot, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{30}-masked-orig.bmp')
        transforms.visualize(one_step, f'{args.outdir}/{args.dataset}/{set_name}/rotation-alias/{j}/{30}-masked-recover.bmp')
    ...
    for j in range(0, 11):
        ...
        orig = loader[0][0]
        orig_mask = rotationT.masking(orig)
        rot = rotationT.raw_proc(orig_mask, (j-5) * 3)
        transforms.visualize(rot, f'{args.outdir}/{args.dataset}/{set_name}/rotation-steps/{0}/{(j-5) * 3}.bmp')
        rot = rotationT.raw_proc(rot, 15)
